...
export default function Sidebar() {
  ...
  // inline placeholder avatar (if /avatar.jpg missing)
  ...
  return (
    <aside className={`snv2 ${collapsed ? "collapsed" : ""} ${heroOpen ? "open" : ""}`}>
      {/* collapse toggle */}
      <button className="snv2-toggle" onClick={() => setCollapsed(!collapsed)} aria-label="Toggle sidebar" />
      ...
      <button className="btn" onClick={() => goto("Test Tech")}>🏠 {!collapsed && <span>Test Tech</span>}</button>
      <button className="btn" onClick={() => goto("superNova_2177")}>✨ {!collapsed && <span>superNova_2177</span>}</button>
      <button className="btn" onClick={() => goto("GLOBALRUNWAY")}>🌍 {!collapsed && <span>GLOBALRUNWAY</span>}</button>

      <div className="divider" />

      {!collapsed && <div className="section">Navigate</div>}
      <button className="btn" onClick={() => goto("Feed")}>📰 {!collapsed && <span>Feed</span>}</button>
      <button className="btn" onClick={() => goto("Chat")}>💬 {!collapsed && <span>Chat</span>}</button>
      <button className="btn" onClick={() => goto("Messages")}>📬 {!collapsed && <span>Messages</span>}</button>
      <button className="btn" onClick={() => goto("Profile")}>👤 {!collapsed && <span>Profile</span>}</button>
      <button className="btn" onClick={() => goto("Proposals")}>📑 {!collapsed && <span>Proposals</span>}</button>
      <button className="btn" onClick={() => goto("Decisions")}>✅ {!collapsed && <span>Decisions</span>}</button>
      <button className="btn" onClick={() => goto("Execution")}>⚙️ {!collapsed && <span>Execution</span>}</button>

      <button className="btn ghost" onClick={() => goto("Coin")}>🪙 {!collapsed && <span>Coin</span>}</button>
      <button className="btn ghost" onClick={() => goto("Forks")}>🍴 {!collapsed && <span>Forks</span>}</button>
      <button className="btn ghost" onClick={() => goto("Remixes")}>🎛️ {!collapsed && <span>Remixes</span>}</button>

      <div className="divider" />

      {!collapsed && <div className="subheader">Premium</div>}
      <button className="btn" onClick={() => goto("Music")}>🎶 {!collapsed && <span>Music</span>}</button>
      <button className="btn" onClick={() => goto("Agents")}>🚀 {!collapsed && <span>Agents</span>}</button>
      <button className="btn" onClick={() => goto("Enter Metaverse")}>🌌 {!collapsed && <span>Enter Metaverse</span>}</button>

      {!collapsed && <div className="caption">Mathematically sucked into a superNova_2177 void — stay tuned for 3D immersion</div>}

      <div className="divider" />
      <button className="btn" onClick={() => goto("Settings")}>⚙️ {!collapsed && <span>Settings</span>}</button>
    </aside>
  );
}
