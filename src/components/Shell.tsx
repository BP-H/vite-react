...
export default function Shell({
  ...
  // close drawer on route change events if your app emits them
  useEffect(() => bus.on("nav:goto", () => setNavOpen(false)), []);

  // inline placeholder avatar for the FAB (used if /avatar.jpg missing)
    ...
    <div className="layout">
      ...
      >
        <img
          ...
          src="/avatar.jpg"
          onError={(e) => {
            (e.currentTarget as HTMLImageElement).src = avatarData;
          }}
          alt="me"
        />
      ...
      >
        <div className="nav-drawer" onClick={(e) => e.stopPropagation()}>
          ...
          <button className="nav-btn" onClick={() => bus.emit("nav:goto", { label: "Test Tech" })}>🏠 Test Tech</button>
          <button className="nav-btn" onClick={() => bus.emit("nav:goto", { label: "superNova_2177" })}>✨ superNova_2177</button>
          <button className="nav-btn" onClick={() => bus.emit("nav:goto", { label: "GLOBALRUNWAY" })}>🌍 GLOBALRUNWAY</button>
          <div className="nav-divider" />
          <button className="nav-btn" onClick={() => setNavOpen(false)}>✕ Close</button>
        </div>
      </div>

      {/* Feed (full-bleed center column) */}
      <main className="content" style={{ padding: 0, width: "100%" }}>
        <Feed onPortal={onPortal} />
      </main>

      {/* Assistant orb (bottom-right) */}
